...
class CTFWriter(Agent):
	def __init__(self, message_definition_path, trace_path):
		super(CTFWriter, self).__init__()
		self.xmls = []
		self.load_message_definition_xml(message_definition_path)
		self.init_babeltrace(trace_path)
		self.init_events()

	def load_message_definition_xml(self, message_definition_path):
		with Silencer():
			xml = mavparse.MAVXML(message_definition_path,"2.0")
		
		for include in xml.include:
			include_path = os.path.join(os.path.dirname(message_definition_path),include)
			self.load_message_definition_xml(include_path)
	...
	def accept(self, msg):
		if hasattr(msg, "timestamp"):
			print(">>",msg.timestamp)
			self.clock.time = msg.timestamp
		else:
			self.clock.time = int(round(time.time()*1000000))
...
if __name__ == "__main__":
	...
	class timestampedDict(dict):
		def __init__(self, timestamp, *args, **kwargs):
			super(timestampedDict, self).__init__(*args,**kwargs)
		...
		def read(self):
			...
			for src, messages in log["messages"].items():
				for timestamp, msg in messages:
					...
					print(timestamp)
					#time.sleep(0.01)
					msg = timestampedDict(int(round(timestamp*1000000)), msg)
					self.emit(msg)
				break
